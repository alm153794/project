...
<head>
    <meta charset="UTF-8">
    <title>Add Birth Record - East Gojjam VIMS</title>
        ...
        <div class="form-container">
            ...
            <% if (request.getMethod().equals("POST")) {
                try {
                    ...
                    Part filePart = request.getPart("child_photo");
                    if (filePart != null && filePart.getSize() > 0) {
                        ...
                        if (fileName != null && !fileName.isEmpty()) {
                            ...
                            String uploadPath = application.getRealPath("/uploads/birth/");
                            File uploadDir = new File(uploadPath);
                            if (!uploadDir.exists()) uploadDir.mkdirs();
                            
                            String filePath = uploadPath + File.separator + newFileName;
                            filePart.write(filePath);
                            photoPath = "uploads/birth/" + newFileName;
                        }
                    }
                    ...
                    String sql = "INSERT INTO birth_records (record_id, child_first_name, child_middle_name, child_last_name, gender, date_of_birth, place_of_birth, wereda, kebele, photo_path, father_full_name, mother_full_name, registration_date, registered_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                    PreparedStatement stmt = conn.prepareStatement(sql);
                    
                    stmt.setString(1, recordId.trim());
                    stmt.setString(2, getFormValue.apply("child_first_name"));
                    stmt.setString(3, getFormValue.apply("child_middle_name"));
                    stmt.setString(4, getFormValue.apply("child_last_name"));
                    stmt.setString(5, getFormValue.apply("gender"));
                    stmt.setString(6, getFormValue.apply("date_of_birth"));
                    stmt.setString(7, getFormValue.apply("place_of_birth"));
                    stmt.setString(8, getFormValue.apply("wereda"));
                    stmt.setString(9, getFormValue.apply("kebele"));
                    stmt.setString(10, photoPath);
                    stmt.setString(11, getFormValue.apply("father_full_name"));
                    stmt.setString(12, getFormValue.apply("mother_full_name"));
                    stmt.setString(13, getFormValue.apply("registration_date"));
                    stmt.setInt(14, (Integer) session.getAttribute("userId"));
                    
                    int result = stmt.executeUpdate();
                    if (result > 0) {
                        out.println("<div class='success-message'>Success: Birth record added successfully!" + (photoPath != null ? " Photo uploaded." : "") + "</div>");
                    } else {
                        out.println("<div class='error-message'>Error: Failed to add birth record!</div>");
                    }
            ...
            <form method="post" class="record-form" enctype="multipart/form-data">
                <div class="form-row">
                    <div class="form-group">
                        <label for="record_id">Birth Record ID:</label>
                        <input type="text" id="record_id" name="record_id" required placeholder="ETH2020001234567">
                    </div>
                    <div class="form-group">
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        <label for="child_photo">Child Photo (Optional):</label>
                        <input type="file" id="child_photo" name="child_photo" accept="image/jpeg,image/jpg,image/png,image/gif" onchange="previewPhoto(this)">
                ...
                <div class="form-actions">
                    <button type="submit" class="btn btn-primary">Add Birth Record</button>
                    <a href="view-birth.jsp" class="btn btn-secondary">View Records</a>
                </div>
            </form>
        </div>
    </div>
