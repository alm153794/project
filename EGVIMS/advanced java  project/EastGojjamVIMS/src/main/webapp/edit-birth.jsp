...
<head>
    <meta charset="UTF-8">
    <title>Edit Birth Record - East Gojjam VIMS</title>
        ...
        <div class="form-container">
            ...
            <% if (request.getMethod().equals("POST")) {
                try {
                    ...
                    String sql = "UPDATE birth_records SET child_first_name=?, child_middle_name=?, child_last_name=?, gender=?, date_of_birth=?, place_of_birth=?, wereda=?, kebele=?, father_full_name=?, mother_full_name=?, registration_date=? WHERE record_id=?";
                    PreparedStatement stmt = conn.prepareStatement(sql);
                    
                    stmt.setString(1, request.getParameter("child_first_name"));
                    stmt.setString(2, request.getParameter("child_middle_name"));
                    stmt.setString(3, request.getParameter("child_last_name"));
                    stmt.setString(4, request.getParameter("gender"));
                    stmt.setString(5, request.getParameter("date_of_birth"));
                    stmt.setString(6, request.getParameter("place_of_birth"));
                    stmt.setString(7, request.getParameter("wereda"));
                    stmt.setString(8, request.getParameter("kebele"));
                    stmt.setString(9, request.getParameter("father_full_name"));
                    stmt.setString(10, request.getParameter("mother_full_name"));
            ...
            try {
                Class.forName("com.mysql.cj.jdbc.Driver");
                Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/East_Gojjam_VIMS_db", "root", "root");
                PreparedStatement stmt = conn.prepareStatement("SELECT * FROM birth_records WHERE record_id = ?");
            ...
            <form method="post" class="record-form">
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" id="child_first_name" name="child_first_name" value="<%= record.getString("child_first_name") %>" required>
                    </div>
                    <div class="form-group">
                        <label for="child_middle_name">Child Middle Name:</label>
                        <input type="text" id="child_middle_name" name="child_middle_name" value="<%= record.getString("child_middle_name") != null ? record.getString("child_middle_name") : "" %>">
                    </div>
                    <div class="form-group">
                        <label for="child_last_name">Child Last Name:</label>
                        <input type="text" id="child_last_name" name="child_last_name" value="<%= record.getString("child_last_name") %>" required>
                    </div>
                </div>
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" id="date_of_birth" name="date_of_birth" value="<%= record.getDate("date_of_birth") %>" required>
                    </div>
                    <div class="form-group">
                        <label for="place_of_birth">Place of Birth:</label>
                        <input type="text" id="place_of_birth" name="place_of_birth" value="<%= record.getString("place_of_birth") %>" required>
                    </div>
                </div>
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="father_full_name" name="father_full_name" value="<%= record.getString("father_full_name") %>" required>
                    </div>
                    <div class="form-group">
                        <label for="mother_full_name">Mother's Full Name:</label>
                        <input type="text" id="mother_full_name" name="mother_full_name" value="<%= record.getString("mother_full_name") %>" required>
                    </div>
                </div>
                ...
                <div class="form-actions">
                    ...
                    <a href="view-birth.jsp" class="btn btn-secondary">Back to List</a>
                    <% if ("admin".equals(role)) { %>
                    <a href="delete-birth.jsp?id=<%= recordId %>" class="btn btn-danger" onclick="return confirm('Are you sure you want to delete this record?')">Delete Record</a>
                    <% } %>
                </div>
            </form>
        </div>
