...
<head>
    <meta charset="UTF-8">
    <title>Birth Records - East Gojjam VIMS</title>
        ...
        <div class="table-container">
            ...
            <div class="table-actions">
                <div>
                    ...
                    <a href="add-birth.jsp" class="btn btn-primary">Add New Record</a>
                    <% } %>
                    <a href="export-birth.jsp" class="btn btn-success">Export CSV</a>
                </div>
                <form method="get" style="display: flex; gap: 0.5rem; align-items: center;">
                    ...
                    <a href="view-birth.jsp" class="btn btn-secondary">Clear</a>
                    <% } %>
                </form>
            </div>
                ...
                <tbody>
                    <%
                        try {
                            ...
                            String sql = "SELECT * FROM birth_records";
                            
                            if (searchTerm != null && !searchTerm.trim().isEmpty()) {
                                sql += " WHERE record_id LIKE ? OR CONCAT(child_first_name, ' ', IFNULL(child_middle_name, ''), ' ', child_last_name) LIKE ? OR father_full_name LIKE ? OR mother_full_name LIKE ?";
                            }
                    ...
                    <tr>
                        ...
                        <td><%= rs.getString("father_full_name") %></td>
                        <td><%= rs.getString("mother_full_name") %></td>
                        <td><%= rs.getDate("registration_date") %></td>
                        <td>
                            <div class="action-buttons">
                                <a href="detail-birth.jsp?id=<%= rs.getString("record_id") %>" class="btn btn-success">View</a>
                                <% if ("data_entry".equals(session.getAttribute("role"))) { %>
                                <a href="edit-birth.jsp?id=<%= rs.getString("record_id") %>" class="btn btn-secondary">Edit</a>
                                <% } %>
                                <% if (!"guest".equals(session.getAttribute("role"))) { %>
                                <a href="certified-document.jsp?type=birth&id=<%= rs.getString("record_id") %>" class="btn btn-primary">Certificate</a>
                                <% } %>
                                <% if ("data_entry".equals(session.getAttribute("role"))) { %>
                                <a href="delete-birth.jsp?id=<%= rs.getString("record_id") %>" class="btn btn-danger" onclick="return confirm('Are you sure?')">Delete</a>
                                <% } %>
                            </div>
                        </td>
                    </tr>
