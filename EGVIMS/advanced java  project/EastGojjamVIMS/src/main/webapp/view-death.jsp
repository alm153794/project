...
<head>
    <meta charset="UTF-8">
    <title>Death Records - East Gojjam VIMS</title>
        ...
        <div class="table-container">
            ...
            <div class="table-actions">
                <div>
                    ...
                    <a href="add-death.jsp" class="btn btn-primary"> Add New Record</a>
                    <% } %>
                    <a href="export-death.jsp" class="btn btn-success"> Export CSV</a>
                </div>
                <form method="get" style="display: flex; gap: 0.5rem; align-items: center;">
                    ...
                    <a href="view-death.jsp" class="btn btn-secondary">Clear</a>
                    <% } %>
                </form>
            </div>
                ...
                <tbody>
                    <%
                        try {
                            ...
                            String sql = "SELECT d.* FROM death_records d";
                            
                            if (searchTerm != null && !searchTerm.trim().isEmpty()) {
                                sql += " WHERE d.record_id LIKE ? OR CONCAT(d.deceased_first_name, ' ', IFNULL(d.deceased_middle_name, ''), ' ', d.deceased_last_name) LIKE ?";
                            }
                    ...
                    <tr>
                        ...
                        <td>
                            <% if (rs.getString("photo_path") != null && !rs.getString("photo_path").isEmpty()) { %>
                                <img src="<%= rs.getString("photo_path") %>" alt="Photo" style="width: 40px; height: 50px; object-fit: cover; border-radius: 5px; border: 1px solid #ddd;">
                            <% } else { %>
                        ...
                        <td><%= rs.getDate("date_of_death") %></td>
                        <td><%= rs.getInt("age_at_death") %></td>
                        <td><%= rs.getString("cause_of_death") != null ? rs.getString("cause_of_death") : "N/A" %></td>
                        <td><%= rs.getDate("registration_date") %></td>
                        <td>
                            <div class="action-buttons">
                                <a href="detail-death.jsp?id=<%= rs.getString("record_id") %>" class="btn btn-success"> View</a>
                                <% if ("data_entry".equals(session.getAttribute("role"))) { %>
                                <a href="edit-death.jsp?id=<%= rs.getString("record_id") %>" class="btn btn-secondary"> Edit</a>
                                <% } %>
                                <% if (!"guest".equals(session.getAttribute("role"))) { %>
                                <a href="certified-document.jsp?type=death&id=<%= rs.getString("record_id") %>" class="btn btn-primary"> Certificate</a>
                                <% } %>
                                <% if ("data_entry".equals(session.getAttribute("role"))) { %>
                                <a href="delete-death.jsp?id=<%= rs.getString("record_id") %>" class="btn btn-danger" onclick="return confirm('Are you sure?')">Delete</a>
                                <% } %>
                            </div>
                        </td>
                    </tr>
