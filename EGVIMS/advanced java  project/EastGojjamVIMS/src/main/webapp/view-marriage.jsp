...
<head>
    <meta charset="UTF-8">
    <title>Marriage Records - East Gojjam VIMS</title>
        ...
        <div class="table-container">
            ...
            <div class="table-actions">
                <div>
                    ...
                    <a href="add-marriage.jsp" class="btn btn-primary"> Add New Record</a>
                    <% } %>
                    <a href="export-marriage.jsp" class="btn btn-success"> Export CSV</a>
                </div>
                <form method="get" style="display: flex; gap: 0.5rem; align-items: center;">
                    ...
                    <a href="view-marriage.jsp" class="btn btn-secondary">Clear</a>
                    <% } %>
                </form>
            </div>
                ...
                <tbody>
                    <%
                        try {
                            ...
                            String sql = "SELECT m.* FROM marriage_records m";
                            
                            if (searchTerm != null && !searchTerm.trim().isEmpty()) {
                                sql += " WHERE m.record_id LIKE ? OR CONCAT(m.groom_first_name, ' ', m.groom_last_name) LIKE ? OR CONCAT(m.bride_first_name, ' ', m.bride_last_name) LIKE ?";
                            }
                    ...
                    <tr>
                        ...
                        <td><%= rs.getString("marriage_place") %></td>
                        <td><%= rs.getDate("registration_date") %></td>
                        <td>
                            <div class="action-buttons">
                                <a href="detail-marriage.jsp?id=<%= rs.getString("record_id") %>" class="btn btn-success"> View</a>
                                <% if ("data_entry".equals(session.getAttribute("role"))) { %>
                                <a href="edit-marriage.jsp?id=<%= rs.getString("record_id") %>" class="btn btn-secondary"> Edit</a>
                                <% } %>
                                <% if (!"guest".equals(session.getAttribute("role"))) { %>
                                <a href="certified-document.jsp?type=marriage&id=<%= rs.getString("record_id") %>" class="btn btn-primary"> Certificate</a>
                                <% } %>
                                <% if ("data_entry".equals(session.getAttribute("role"))) { %>
                                <a href="delete-marriage.jsp?id=<%= rs.getString("record_id") %>" class="btn btn-danger" onclick="return confirm('Are you sure?')">Delete</a>
                                <% } %>
                            </div>
                        </td>
                    </tr>
